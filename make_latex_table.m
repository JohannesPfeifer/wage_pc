function make_latex_table(fname,title,LaTeXtitle,headers,labels,values,label_width,val_width,val_precis,optional_header)
%function make_latex_table(fname,title,LaTeXtitle,headers,labels,values,label_width,val_width,val_precis,optional_header)
...
filename = [OutputDirectoryName '/' fname '_' LaTeXtitle '.tex'];
fidTeX = fopen(filename,'w');

stack = dbstack;
fprintf(fidTeX,['%% ' datestr(now,0) ', created by Wage_PC_Calvo_Rotemberg']);
fprintf(fidTeX,' \n');
fprintf(fidTeX,' \n');
